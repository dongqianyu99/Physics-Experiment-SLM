...
%{
r = N * res / 2;
inside_lens = x0^2 + y0^2 <= r^2;
d = zeros(N,N);
d(inside_lens) = 2 * (r - sqrt(r^2 - x0(inside_lens).^2 - y0(inside_lens).^2));
Ein = Ain * exp(-1i*(pi/(lam))+d);
%}
...
%Ein = Ain*(0.5 + 0.5*cos(4*pi*(x0.^2 + y0.^2)));

Nslid =  1000;
zd = 0.2+linspace(0,1.0,1000);
A1 = zeros(N,N,Nslid);
Exy = zeros(N,N,Nslid);
Eyz = zeros(N,Nslid);
figure('color','white')
