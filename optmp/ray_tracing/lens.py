...
class Lens(object):  # 添加透镜
	surface_list = []
	def __init__(self,lens_name='',creator=''):
		self.lens_name = lens_name
		self.creator = creator
		self.field_angle_list = []  # 视场角
		self.wavelength_list = []  # 透镜波长
		self.EP_thickness = 0
		self.EFL = 0  
		self.EPD = 0
		self.FNO = 0
	...
	def refresh_paraxial(self):
		self.EFL = first_order_tools.EFL(self,0,0)  # 有效焦距（EFL，Effective Focal Length）
		self.EPD = self.EFL/self.FNO  # 入瞳直径（EPD，Entrance Pupil Diameter）
