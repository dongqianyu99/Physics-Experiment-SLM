...
class Surface(object):  # 定义透镜表面（？）
    ...
    def __init__(self,wavelength_list,number,radius,thickness,glass,STO,__diameter__):
        ...
        self.radius = radius  # 曲率半径
        self.glass = glass  # 玻璃材质（啊？）
        # self.indexlist = glass_funcs.glass2indexlist(wavelength_list,glass)  # 获取该玻璃材料在不同波长下的折射率列表
        if glass == 'air':
            self.indexlist = 1.0
        else:
            self.indexlist = 1.5
