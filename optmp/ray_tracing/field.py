...
class Ray(object):  # 光线
    def __init__(self,Pos,KLM):  # Pos位置，KLM方向
        '''
        Pos: ray position
        KLM: ray direction
        '''
        self.Pos = __np__.asarray(Pos)  # "asarray" numpy数组类型
        self.KLM = __np__.asarray(KLM)
    def list(self):
        print("Ray Position :",self.Pos)
        print("Ray Direction:",self.KLM)

class Field(object):  # 光场（就是多束光线的集合）
    def __init__(self,Raylist):
        self.ray_list = []
        for i in Raylist:
            self.ray_list.append(Ray(i[0],i[1]))  # 转换为Ray对象列表进行存储

def add_field_YAN(Lens,angle):  # 向光场中添加光线（给定的是出射角度？）
    '''
